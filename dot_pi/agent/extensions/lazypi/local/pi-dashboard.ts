...
function syncDashboardHeader(): void {
	if (!currentCtx) return;
	if (agentViewDepth > 0) installEmptyHeader(currentCtx);
	else installDashboardHeader(currentCtx);
}
...
export default function piDashboard(pi: ExtensionAPI) {
	...
	pi.events.on(AGENT_VIEW_OPEN_EVENT, () => {
		agentViewDepth++;
		syncDashboardHeader();
	});

	pi.events.on(AGENT_VIEW_CLOSE_EVENT, () => {
